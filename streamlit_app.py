...
Обучаемые ИИ агенты | Сохранение | Русские условия | Google Sheets + AI | Excel AI
================================================================================
"""
...
class GoogleSheetsManager:
    ...
    def append_row(self, row_data: List) -> Dict:
        try:
            ...
            return {"success": True, "row": row_data, "row_number": len(self.worksheet.get_all_values()) + 1,
    ...
    def batch_update_cells(self, updates: List[Dict]) -> Dict:
        try:
            cells = []
            for upd in updates:
                cell = upd.get('cell')
                value = upd.get('value')
                if cell:
                    cells.append({'range': cell, 'values': [[str(value)]]})
            if cells:
                self.worksheet.batch_update([{'range': c['range'], 'values': c['values']} for c in cells])
...
# ============================================================================
# ИСПОЛНИТЕЛЬ WORKFLOW (РАСШИРЕННЫЙ)
# ============================================================================
    ...
    def _execute_google_sheets_read(self, config: Dict) -> Dict:
        ...
        try:
            ...
            if self.gs_manager.open_sheet(sheet_url, config.get('worksheet_name')):
                ...
                if config.get('analyze_with_ai') and self.api_key:
                    prompt = config.get('ai_prompt', 'Проанализируй данные')
                    ...
                    result['ai_analysis'] = resp.choices[0].message.content
                return result
                ...
                for col_name, source in column_mapping.items():
                    ...
                    if 0 <= col_idx < max_col:
                        if source.startswith('{{') and source.endswith('}}'):
                            ...
                            if config.get('ai_gen') and var_name in config.get('ai_gen_prompt', ''):
                                prompt = config['ai_gen_prompt'].replace('{{value}}', str(val))
                                client = OpenAI(api_key=self.api_key, base_url="https://api.deepseek.com/v1")
                                resp = client.chat.completions.create(
                                    model="deepseek-chat",
...
with st.sidebar:
    ...
    st.markdown("## 📌 Мои условия")
    with st.expander("➕ Сохранить условие"):
        cond_name = st.text_input("Название условия", placeholder="Проверка цены")
        cond_text = st.text_area("Текст условия", placeholder="если цена больше 1000 то отправить уведомление")
        if st.button("💾 Сохранить условие"):
            if cond_name and cond_text:
                st.session_state.custom_conditions.append({"name": cond_name, "text": cond_text})
                st.success("Сохранено!")
                st.rerun()
    if st.session_state.custom_conditions:
        for i, cond in enumerate(st.session_state.custom_conditions):
            st.markdown(f"- **{cond['name']}** `{cond['text'][:30]}...`")
...
tabs = st.tabs([
    ...
    "🤖 Workflow", "🔀 Русские условия", "📧 Email/Google", "🎤 Голосовой",
    "📁 Excel AI"
])
        ...
        if current_agent.training_examples:
            for i, example in enumerate(reversed(current_agent.training_examples[-10:])):
                ...
                <div class="training-example">
                    ...
                    <small>📅 {example['timestamp'][:10]}</small>
                </div>
                """, unsafe_allow_html=True)
        ...
        if st.session_state.workflow:
            for i, block in enumerate(st.session_state.workflow):
                ...
                with st.expander(f"⚙️ Настроить {block.get('name', 'Block')}"):
                    ...
                    if block_type == 'google_sheets_read':
                        ...
                        config['analyze_with_ai'] = st.checkbox("Анализ ИИ", config.get('analyze_with_ai', False), key=f"ai_{i}")
                        config['ai_prompt'] = st.text_area("Промпт анализа", config.get('ai_prompt',''), key=f"ai_prompt_{i}")
                        ...
                        config['ai_gen'] = st.checkbox("Генерация значений ИИ", config.get('ai_gen', False), key=f"ai_gen_{i}")
                        config['ai_gen_prompt'] = st.text_area("Промпт генерации", config.get('ai_gen_prompt',''), key=f"ai_gen_prompt_{i}")
                        ...
                        options = [c['text'] for c in st.session_state.custom_conditions]
                        selected = ""
                        if options:
                            selected = st.selectbox("Сохранённые", [""]+options, key=f"selcond_{i}")
                            if selected:
            ...
            if st.button("🚀 ЗАПУСТИТЬ WORKFLOW", type="primary", use_container_width=True):
                ...
                for block in st.session_state.workflow:
                    ...
                    if block.get('type') in ['email_send', 'outlook_send']:
...
with tabs[5]:
    ...
    with col1:
        ...
        test_condition = st.text_area("Напишите условие:", height=150,
    ...
    with col2:
        st.markdown("### 💾 Сохранённые условия")
        if st.session_state.custom_conditions:
            for c in st.session_state.custom_conditions:
                st.code(f"{c['name']}: {c['text']}")
        else:
            st.info("Нет сохранённых условий. Добавьте в боковой панели.")
...
with tabs[6]:
    ...
    with tab_gsheets:
        st.subheader("📊 Google Sheets + AI")
        if not credentials_json:
            st.warning("⚠️ Для работы с Google Sheets необходим JSON ключ сервисного аккаунта. Добавьте его в боковой панели.")
        else:
            url = st.text_input("URL таблицы", placeholder="https://docs.google.com/spreadsheets/d/...")
            sheet = st.text_input("Лист (опционально)")
            if st.button("📖 Прочитать и проанализировать"):
                gs = GoogleSheetsManager(credentials_json)
                if gs.open_sheet(url, sheet):
                    data = gs.get_all_data()
                    st.dataframe(pd.DataFrame(data))
                    if api_key:
                        prompt = st.text_area("Промпт для анализа", "Найди среднее арифметическое второго столбца")
...
with tabs[7]:
    ...
    if st.button("🎤 НАЧАТЬ ЗАПИСЬ", type="primary"):
...
# ============================================================================
with tabs[8]:
    st.subheader("📁 Обработка Excel через ИИ")
    uploaded_file = st.file_uploader("Загрузите Excel файл", type=['xlsx'])
    if uploaded_file:
        df = pd.read_excel(uploaded_file, engine='openpyxl')
        st.dataframe(df.head())
        sheet_name = st.selectbox("Лист", pd.ExcelFile(uploaded_file, engine='openpyxl').sheet_names)
        df = pd.read_excel(uploaded_file, sheet_name=sheet_name, engine='openpyxl')
        col = st.selectbox("Столбец для обработки", df.columns)
        prompt = st.text_input("Промпт для ИИ", "Переведи на английский")
        new_col = st.text_input("Имя нового столбца (оставьте пустым для замены)")
        if st.button("🚀 Запустить ИИ обработку") and api_key:
            ...
            st.download_button("📥 Скачать результат", data=output.getvalue(), file_name="processed.xlsx")
